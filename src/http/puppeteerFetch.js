...
/** Errors that indicate the browser process crashed and should be relaunched. */
const BROWSER_CRASH = /TargetCloseError|Protocol error|Target closed|Connection closed|Session closed|pipe.*closed|browser.*disconnect/i;
 ...
 */
async function fetchWithPuppeteer(url, torPort, referrer, _retried = false) {
    let context;
    let page;
    try {
        ...
        if (!_retried && BROWSER_CRASH.test(error.message)) {
            log.warn(`Puppeteer browser crashed — relaunching and retrying ${url}`);
            browser = null;
            return fetchWithPuppeteer(url, torPort, referrer, true);
        }
